...
void EquipManager::equipPokemon(const std::shared_ptr<Equipment> &equipment,
                                const std::shared_ptr<Pokemon> &pokemon)
{
    if (equipment->type == Equipment::ACCESSORY)
    {
        std::shared_ptr<Accessory> newAccessory = std::dynamic_pointer_cast<Accessory>(
                equipment);
    ...
    {
        std::shared_ptr<Armor> newArmor = std::dynamic_pointer_cast<Armor>(
                equipment);
        if (pokemon->armor)
        {
...
void EquipManager::unequipPokemon(EquipType eType,
                                  const std::shared_ptr<Pokemon> &pokemon)
{
    if (eType == EquipType::Accessory)
    {
        ...
        {
            pokemon->accessory->removeEffect(pokemon);
            std::cout << pokemon->name << " 卸下了 " << pokemon->accessory->name
                      << "。" << std::endl;
